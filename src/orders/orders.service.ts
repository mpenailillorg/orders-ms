...
export class OrdersService extends PrismaClient implements OnModuleInit {
  ...
  async create(createOrderDto: CreateOrderDto) {
    try {
      ...
      const products = await firstValueFrom(this.client.send({ cmd: 'validate_products' }, productsId)) // Obtengo los productos asociados a esos ids
  ...
  async findOne(id: string) {
    ...
    const products = await firstValueFrom(this.client.send({ cmd: 'validate_products' }, order.OrderItem.map(orderItem => orderItem.productId)));
